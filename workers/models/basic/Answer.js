module.exports = function(config, mongoose, models) {
  ...
  var AnswerSchema = new mongoose.Schema({
    ...
    accepted: {type: Boolean},  // if the answer is accepted by the asker
  ...
  AnswerSchema.pre('save', function(next){
    ...
    if ( !this.created_at ) {
      ...
      this.permalink = "/answer/"+this.vendor+"/"+procat+"/"+this.pub_id;

      // var answer = new Answer(answerObj);
      ...
      models.Seller.Model.findOne({
        site_domain: this._seller.site_domain
          ...
          if (!seller.meta_info.answer_count) {
            seller.save(function(err, saved_seller){
              console.log("seller.meta_info.answer_count", saved_seller.meta_info.answer_count);
              saved_seller.meta_info.answer_count = 1
              saved_seller.save();
            })
          } else {
            seller.meta_info.answer_count += 1
            seller.save();
          }
      ...
      for (var i = 0; i < pscats_arr.length; ++i) {
        ...
        models.ProductCat.Model.findOne({
          ...
          if (!procat.meta_info.answer_count) {
            console.log("#####################################################################################")
            procat.save(function(err, saved_procat){
              console.log("###########$$##########################################################################")
              console.log("procat.meta_info.answer_count", saved_procat.meta_info.answer_count);
              saved_procat.meta_info.answer_count = 1
              saved_procat.save();
            })
          } else {
            console.log("###########@@##########################################################################")
            procat.meta_info.answer_count += 1
            procat.save();
          }
        });          
      }
  ...
  AnswerSchema.post('save', function(answer_doc){
    ...
    // #TODO write a function for adding the answer pub_id to Seller in Seller Model
    models.Seller.Model.findOne({pub_id: answer_doc._seller.pub_id}, function(err, seller_doc){
      if (!err && seller_doc){
        seller_doc._answers_pub_id.push(answer_doc.pub_id);
        seller_doc.save();
        console.log("@@@@@@@ answer id pushed in seller")
      }
    })

    // #TODO write a function for adding the answer pub_id to Seller in ProductCat Model
    for (var i = 0; i < answer_doc._pro_cat.pub_id.length; ++i){
      models.ProductCat.Model.findOne({pub_id: answer_doc._pro_cat.pub_id[i]}, function(err, procat_doc){
        if (!err && procat_doc){
          procat_doc._answers_pub_id.push(answer_doc.pub_id);
          procat_doc.save();
          console.log("@@@@@@@ answer id pushed in procat num:", i)
        }
      });
    }
