module.exports = function(config, mongoose, models, modelName) {
  ...
  var ServiceCatSchema = new mongoose.Schema({
    ...
    wiki_url: { type: String }, // url of the wikipedia page with more detailed information about the service category
    google_url: { type: String }, // url of the Google Search results page with more information about the service category
    picture: { type: String }, // image url 

    sample_service_url: { type: [String] }, // url of few sample services in this category from each vendors
    sample_service_images: { type: [String] }, // image url for them
    ...
    _child_ser_cat: [{ type: Number, ref: 'ServiceCat', index: true }],
    _parent_ser_cat: [{ type: Number, ref: 'ServiceCat', index: true }],
    _ser: [{ type: Number, ref: 'Service', index: true }],

    // Related Products Categories with this Service Category
    _related_pro_cat: [{ type: Number, ref: 'ProductCat', index: true }],
    
    // Related Products with this Service Category
    _related_pro: [{ type: Number, ref: 'Product', index: true }],
  ...
  ServiceCatSchema.pre('save', function(next){
    ...
    if (this._id) {
      var plink = "/service-category/"+this._id+"/"+this.url_name;
    ...
    if ( !this.created_at ) {
      ...
      this.node_type = "_ser_cat";
      this.node_type_num = "2";
      this.meta_info = {
  ...
  // var ServiceCat = mongoose.model('ServiceCat', ServiceCatSchema);
  var ServiceCat = mongoose.model(modelName, ServiceCatSchema);
