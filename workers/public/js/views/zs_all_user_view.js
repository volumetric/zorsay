define(['Backbone', 'bootstrap', 'views/zs_notify_view'], function(B, b, zsNotifyView) {
  console.log("inside zs_all_user_view")

  var zsAllUserView = Backbone.View.extend({
    el: 'body',
    ...
    initialize: function() {
      console.log("zs all user view initialized");
    ...
    load_more: function() {
      ...
      $.ajax({
        type: 'GET',
        url: '/users/'+order+'/'+lastId+'/'+limit+'/html',
        success: function(data, status, jqxhr) {
          // console.log(jqxhr.status);
          $(".question_short_list").append(data);
          zsNotifyView.nv_hide(1);
