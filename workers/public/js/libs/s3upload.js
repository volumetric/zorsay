  ...
  window.S3Upload = (function() {
    ...
    function S3Upload(options) {
      ...
      if (this.file_dom_element) {
        this.handleFileSelect(this.file_dom_element, this.file_opts);
      } else if (this.file_dom_selector) {
        this.handleFileSelect($(this.file_dom_selector).get(0), this.file_opts);
      }
    }

    S3Upload.prototype.handleFileSelect = function(file_element, file_opts) {
      ...
      for (_i = 0, _len = files.length; _i < _len; _i++) {
        f = files[_i];
        _results.push(this.uploadFile(f, file_opts));
    ...
    S3Upload.prototype.executeOnSignedUrl = function(file, callback, opts) {
      ...
      // console.log("sending for signed url");
      // console.log(opts);
      xhr.open('GET', this.s3_sign_put_url + '?s3_object_type=' + type + '&s3_object_name=' + encodeURIComponent(name) + '&opts=' + opts, true);
