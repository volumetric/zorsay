module.exports = function(config, mongoose, models, Seller, Shopex) {
  ...
  ProductCatSchema.pre('save', function(next){
    ...
    if ( !this.created_at ) {
      ...
      // var mp = new MetaProductCat(mpObj);
  ...
  var getIdByQuery = function(query, callback) {
    console.log("%%% inside getIdByQuery ProductCat %%%")
    ProductCat.findOne(query, function(err, porcat){
      callback(err, porcat);
    });
  }

  var getProductCatsByShopexCountSkipLimit = function(skip, limit, callback) {
    ProductCat.find().sort({
  ...
  return {
    ...
    getProductCatsByShopexCountSkipLimit: getProductCatsByShopexCountSkipLimit,
    // register: register,
